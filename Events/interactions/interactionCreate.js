...
const subCommand = interaction.options.getSubcommand(false);
    ...
    if (subCommand) {
      ...
      if (command.moderatorOnly) {
        if (
          !member.roles.cache.has("1091147283036373062") ||
          !member.roles.cache.has("1091147283036373062") ||
          !member.roles.cache.has("1091147283036373062") ||
          !member.roles.cache.has("1091147283036373062")
        ) {
    ...
    if (interaction.isModalSubmit()) {
      if (customId == "developerModal") {
        const channel = await client.channels.cache.get("1091125286361366550");
          ...
          GitHubSchema.findOne(
            ...
            async (err, data2) => {
              ...
              const embed = new EmbedBuilder()
                .setAuthor({
                  name: "Access Request",
                  iconURL: "https://kajdev.org/img/400x400_logo.png",
                })
                .setDescription(
